...
import configparser
from ui.fivesixmonths_edit import Ui_fivesixmonths
import urls
import time
import dict_const
...
SLCS=['正常','无','不配合','未检查']
JCHZ=['是','否']
YXJY=['定期眼科检查：下次检查时间：1岁','眼科门诊就诊，进一步检查治疗']
JCJL=['所查项目未见异常','其它：']
YWJC=['正位','斜视','未检查']
YQZCJC=['无震颤','有震颤','未检查']
...
class Edit(QDialog, Ui_fivesixmonths): #显示
    ...
    def on_pushButton_save_clicked(self):
        ...
        from contorler import Fivesixmonths
        res1 = self.archives_save()
        if res1['bool']:
            res2 = self.save_check_record()
            self.Fivesixmonths = Fivesixmonths.Display(user_type=u'医生', archives_info_from_eidt=res1['type'],check_info_from_eidt=res2)
            self.Fivesixmonths.setWindowModality(Qt.ApplicationModal)  # 只能编辑当前弹出的页面
            self.Fivesixmonths.show()
            result = self.Fivesixmonths.exec_()

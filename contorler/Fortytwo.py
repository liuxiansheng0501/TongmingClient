...
from ui.fortytwodays import Ui_fortytwodays
import urls

class Display(QDialog, Ui_fortytwodays): #显示
    ...
    def checked_result_info(self):
        if self.archives_info_from_eidt == '':  # 统计入口点击
            res=requests.get(url=urls.get_42days, data=self.info_stat)
            if res.ok:
    ...
    def display(self):
        ...
        self.label_slcs.setText(self.result_info['eye_sight_first_time_filter'])
    ...
    def save_check_record(self):
        res = requests.post(urls.save_42days, data=self.result_info)
        if res.ok:
            return True
        else:
            return False

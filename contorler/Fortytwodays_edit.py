...
import configparser
from ui.fortytwodays_edit import Ui_fortytwodays
import urls
import time
import dict_const
...
SLCS=['正常','无','不配合','未检查']
JCHZ=['是','否']
YXJY=['定期眼科检查:下次检查时间:5-6个月','眼科门诊就诊，进一步检查治疗']
JCJL=['所查项目未见异常','其它：']


class Edit(QDialog, Ui_fortytwodays): #显示
    ...
    def initial_check_info(self):
        ...
        self.comboBox_slcs.addItems(SLCS)
    ...
    def archives_save(self):#若第一次录入档案信息新建档案并保存
        if not self.is_base_archives_valid():
            res=QMessageBox.information(self, '提示', '请按要求填写儿童基本档案信息')
    ...
    def save_check_record(self):
        ...
        eye_sight_first_time_filter = self.comboBox_slcs.currentText()
        ...
        save_contents = {'appoint_datetime': appoint_datetime,
                         ...
                         'eye_sight_first_time_filter': eye_sight_first_time_filter,
        ...
        res = requests.post(urls.save_42days, data=save_contents)
    ...
    def on_pushButton_save_clicked(self):
        ...
        from contorler import Fortytwo
        res1 = self.archives_save()
        if res1['bool']:
            res2 = self.save_check_record()
            self.Fortytwo = Fortytwo.Display(user_type=u'医生', archives_info_from_eidt=res1['type'],check_info_from_eidt=res2)
            self.Fortytwo.setWindowModality(Qt.ApplicationModal)  # 只能编辑当前弹出的页面
            self.Fortytwo.show()
            result = self.Fortytwo.exec_()

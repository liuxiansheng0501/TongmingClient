...
from ui.newborn import Ui_newborn
import urls

class Display(QDialog, Ui_newborn): #显示
    ...
    def checked_result_info(self):
        if self.archives_info_from_eidt == '':  # 统计入口点击
            res=requests.get(url=urls.get_newborns, data=self.info_stat)
            if res.ok:
    ...
    def display(self):
        ...
        self.label_gzfy_left.setText(self.result_info['light_response_left'])
        self.label_gzfy_right.setText(self.result_info['light_response_right'])
        ...
        self.label_tkdgfs_left.setText(self.result_info['pupil_reflects_on_light_left'])
        self.label_tkdgfs_right.setText(self.result_info['pupil_reflects_on_light_right'])
    ...
    def save_check_record(self):
        res = requests.post(urls.update_newborns, data=self.result_info)
        if res.ok:
            return True
        else:
            return False

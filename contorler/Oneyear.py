...
from ui.oneyear import Ui_oneyear
import urls

class Display(QDialog, Ui_oneyear): #显示
    ...
    def checked_result_info(self):
        if self.archives_info_from_eidt == '':  # 统计入口点击
            res = requests.get(url=urls.get_1year, data=self.info_stat)
            if res.ok:
    ...
    def save_check_record(self):
        res = requests.post(urls.save_1year, data=self.result_info)
        if res.ok:
            return True
        else:
            return False

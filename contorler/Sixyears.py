...
from ui.sixyears import Ui_sixyears
import urls

class Display(QDialog, Ui_sixyears): #显示
    ...
    def checked_result_info(self):
        if self.archives_info_from_eidt == '':  # 统计入口点击
            res = requests.get(url=urls.get_6year, data=self.info_stat)
            if res.ok:
    ...
    def display(self):
        ...
        self.label_wyjyqjjc.setText(self.result_info['outer_eye_anterior_segment_check'])
    ...
    def save_check_record(self):
        res = requests.post(urls.save_6years, data=self.result_info)
        if res.ok:
            return True
        else:
            return False

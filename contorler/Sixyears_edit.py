...
import configparser
from ui.sixyears_edit import Ui_sixyears
import urls
import time
import dict_const
...
class Edit(QDialog, Ui_sixyears): #显示
    ...
    def on_pushButton_save_clicked(self):
        ...
        from contorler import Sixyears
        res1 = self.archives_save()
        if res1['bool']:
            res2 = self.save_check_record()
            self.Sixyears = Sixyears.Display(user_type=u'医生', archives_info_from_eidt=res1['type'],check_info_from_eidt=res2)
            self.Sixyears.setWindowModality(Qt.ApplicationModal)  # 只能编辑当前弹出的页面
            self.Sixyears.show()
            result = self.Sixyears.exec_()

...
from ui.threeyears import Ui_threeyears
import urls

class Display(QDialog, Ui_threeyears): #显示
    ...
    def checked_result_info(self):
        if self.archives_info_from_eidt == '':  # 统计入口点击
            res = requests.get(url=urls.get_3year, data=self.info_stat)
            if res.ok:
    ...
    def display(self):
        ...
        self.label_yyesrdjc_left.setText(self.result_info['infant_vision_acuity_check_left'])
        self.label_yyesrdjc_right.setText(self.result_info['infant_vision_acuity_check_right'])
        ...
        self.label_ybcs.setText(self.result_info['eye_x_check'])
    ...
    def save_check_record(self):
        res = requests.post(urls.save_3years, data=self.result_info)
        if res.ok:
            return True
        else:
            return False

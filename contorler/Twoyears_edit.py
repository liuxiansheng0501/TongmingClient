...
import configparser
from ui.twoyears_edit import Ui_twoyears
import urls
import time
import dict_const
...
SLCS=['正常','无','不配合','未检查']
JCHZ=['是','否']
YXJY=['定期眼科检查：下次检查时间：3岁', '眼科门诊就诊，进一步检查治疗']
JCJL=['所查项目未见异常','其它：']
YWJC=['正位','斜视','未检查']
YQZCJC=['无震颤','有震颤','未检查']
...
class Edit(QDialog, Ui_twoyears): #显示
    ...
    def initial_check_info(self):
        ...
        self.comboBox_yyesrdjc_right.addItems(YYESRDJC)
        self.comboBox_yyesrdjc_left.addItems(YYESRDJC)

        self.comboBox_yyexzxzs_right.addItems(YYEXZXZS)
        self.comboBox_yyexzxzs_left.addItems(YYEXZXZS)
    ...
    def save_check_record(self):
        ...
        infants_selective_gaze_left = self.comboBox_yyexzxzs_left.currentText()
        infants_selective_gaze_right= self.comboBox_yyexzxzs_right.currentText()

        dot_vision_check_left = self.comboBox_dzsljc_left.currentText()
        dot_vision_check_right = self.comboBox_dzsljc_right.currentText()

        infant_vision_acuity_check_left=self.comboBox_yyesrdjc_left.currentText()
        infant_vision_acuity_check_right = self.comboBox_yyesrdjc_right.currentText()
        ...
        eye_x_check=self.textEdit_ybcs.toPlainText()
        ...
        save_contents = {'appoint_datetime': appoint_datetime,
                         ...
                         'infants_selective_gaze_left': infants_selective_gaze_left,
                         'infants_selective_gaze_right': infants_selective_gaze_right,
                         'dot_vision_check_left': dot_vision_check_left,
                         'dot_vision_check_right': dot_vision_check_right,
                         'infant_vision_acuity_check_left':infant_vision_acuity_check_left,
                         'infant_vision_acuity_check_right': infant_vision_acuity_check_right,
    ...
    def on_pushButton_save_clicked(self):
        ...
        from contorler import Twoyears
        res1 = self.archives_save()
        if res1['bool']:
            res2 = self.save_check_record()
            self.Twoyears = Twoyears.Display(user_type=u'医生', archives_info_from_eidt=res1['type'],check_info_from_eidt=res2)
            self.Twoyears.setWindowModality(Qt.ApplicationModal)  # 只能编辑当前弹出的页面
            self.Twoyears.show()
            result = self.Twoyears.exec_()
